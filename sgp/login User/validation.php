...
if (isset($_POST['sin_username']) && isset($_POST['sin_password'])) {
    ...
    if ($num ==1) {
        ...
        if($num1==1){
            $conn->close();
            header('location:/sgp/Login User/worker.php'); //this is for company man

        }
        else{
            $conn->close();
            header('location:/sgp/Login User/index2.html'); //this is customer
        }
    }
    else{
        $conn->close();
        header('location:/sgp/Login User/Login.php');
    }
}
